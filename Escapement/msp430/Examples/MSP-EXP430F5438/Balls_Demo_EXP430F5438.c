...
** -------------------------------
** 1. Run EscapementConf.exe and open the configuration file Balls_Demo_EXP430F5438.zot at
...
**    At step 2, select the kernel (Escapement-Hard or Escapement-Soft)
**    Go to 7 and generate files
**        Escapement_msp430f5419A_36A_38A.h
**    and
**        Escapement_msp430f5419A_36A_38A.asm
** 2. If you selected Escapement-Hard, edit file EscapementHard.h and set the symbolic constant
...
** version of the Escapement kernel ("EscapementHard Demo" or "EscapementSoft Demo"). The top line
...
** applied to the microcontroller, the current processor load, and for Escapement-Soft, the
...
** S2 is only functional with Escapement-Soft and cyclically steps through higher workloads
...
** nector. This additional functionality uses the UART API of Escapement. The user can then
...
**  12. The synthetic task that wastes CPU cycles to show the benefit of Escapement-Soft
...
**   SyntheticTask            9300     15000    1   1        (only with Escapement-Soft)
...
**           contraints;
**        2. Containts are given for Escapement-Soft but under Escapement-Hard, these are equi-
**           valent to (1,1);
...
#include <string.h>
#include "LCD_EXP430F5438/hal_lcd_Escapement.h"

/* Sanity Test */
#if !defined(ESCAPEMENT_VERSION_HARD) && !defined(ESCAPEMENT_VERSION_SOFT)
...
int main(void)
{
  ...
  OSInitializeSystemClocks(); // This function is created by EscapementConf.exe
...
** line of the screen only shows the kernel version of Escapement. Once the user switches to
** the 2nd page, the current time with 5 characters is added to the right:
**      Column number:    01234567890123456
**      Displayed text:   EscapementHard 00:00
...
*/ 
   ...
   #define BANNER_STRING  "EscapementHard      " /* 1st part of the text */
#elif defined(ESCAPEMENT_VERSION_SOFT)
   #define BANNER_STRING  "EscapementSoft      "
#endif
...
void InitBanner(void)
{
  ...
  #elif defined(ESCAPEMENT_VERSION_SOFT)
     #error Untested with Escapement-Soft
  #else
     #error Wrong kernel version
  #endif
} /* end of InitBanner */
...
** P2.7.
** Although the synthetic task is created only for Escapement-Soft, the interrupt handler
** for S2 is still installed in Escapement-Hard to reactivate the LCD screen when S2 is
** pressed. */
