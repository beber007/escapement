...
*/
/* File EscapementSoft.h: Defines the user interface with EscapementSoft.c.
...
** (3) Create application tasks (this step requires more parameters than EscapementHard).
...
**          // the value 34 as a parameter. This task has a worst-case execution time of
**          // 200, a period and deadline of 10000 clock-ticks. The 5th and 6th parameter
**          // define the firm deadlines of the task: out of 3 consecutive instances, at
**          // least 2 must succeed their deadlines. The 7th parameter defines the first
**          // task instance. In this example, there is no initial lag when starting the
**          // task as the very first instance is number 0. (Task instances are numbered
**          // cyclically from 0 to 2.
...
**       int main(void)
**       {
**          // Create a periodic task that triggers an event. (Assume that this task re-
**          // quires 2000 ticks. (Loads the processor by at most 20%.)
**          OSCreateTask(SignalerTask,2000,0,10000,10000,1,1,0,NULL);
**          // Create an event that will be triggered by SignalerTask
**          Event = OSCreateEventDescriptor();
**          // Create an aperiodic task that processes event Event, (Assume that this
**          // task requires 4000 and should complete before 8000 once it is triggered.
**          // (Loads the processor by at most 50%.)
**          #if SCHEDULER_REAL_TIME_MODE == EARLIEST_DEADLINE_FIRST
**             OSCreateSynchronousTask(AperiodicTask,4000,0,128,Event,NULL);
**          #else
**             OSCreateSynchronousTask(AperiodicTask,4000,8000,0,Event,NULL);
**          #endif
...
/* Escapement can operate under 2 possible scheduling algorithms. Under EDF, tasks are sche-
...
** TERNAL_HEAP_SIZE and can be found in Escapement_Config.h. */
...
**       task; this parameter is only used if there are some periodic tasks that have
**       (m,k)-hard constraints with m != k and can be set to 0 when no such task exists.
...
**   (6) (UINT8) m: Used for (m,k)-firm guarantees, i.e. out of k consecutive instances,
**                  there should be m instances that meet their deadlines, m must be > 0.
**   (7) (UINT8) k: See above. k must be >= m. When k = m, all deadlines must their dead-
**                  lines.
...
** Returned value: (UINT8) This is a number comprised in the interval [0..k-1], where k
**   is the number of consecutive task instances that was passed as parameter to
**   OSCreateTask(). */
...
** of this table, we recommend using Escapement MSP430 Configurator Tool, which allows you
...
** valid entries to the internal table can be found in the generated Escapement_msp430XXX.h
** or Escapement_cc430XXX.h file.
*/
