...
;
; File Escapement_msp430x552x.asm: Holds msp430x552x dependent assembler implemented
;                               functions.
; Created on March 12, 2010, by Escapement MSP430 Configuration Tool.
; Include device definition file and Escapement configuration file
  .cdecls C,LIST,"Escapement_msp430x552x.h"


; ******************************************************
; Global Variables Shared with Escapement.c and Escapement.asm
; ******************************************************
...
; *********************************
; Imported Functions From Escapement.c
; *********************************
...
; Timer0_A (int52): Interval timer0 A interrupt handler used by Escapement to propagate
...
; USB interrupt handlers (int51).
  ...
  mov #OS_IO_USB_SETUP_PACKET_RECEIVED,r13  ; Load interrupt entry to Escapement device handler table in R13
  jmp EndUSB
USB_CONTINUE:
  ...
  eint                                ; Enable all interrupts
  mov #OS_IO_USB_OUTPUT_ENDPOINT7,r13 ; Load interrupt entry to Escapement device handler table in R13
  jmp EndUSB
USB_OUTPUT_ENDPOINT6:
  bic.b #BIT6,&USBOEPIE               ; Disable Output Endpoint-6 interrupt
  eint                                ; Enable all interrupts
  mov #OS_IO_USB_OUTPUT_ENDPOINT6,r13 ; Load interrupt entry to Escapement device handler table in R13
  jmp EndUSB
USB_OUTPUT_ENDPOINT5:
  bic.b #BIT5,&USBOEPIE               ; Disable Output Endpoint-5 interrupt
  eint                                ; Enable all interrupts
  mov #OS_IO_USB_OUTPUT_ENDPOINT5,r13 ; Load interrupt entry to Escapement device handler table in R13
  jmp EndUSB
USB_OUTPUT_ENDPOINT4:
  bic.b #BIT4,&USBOEPIE               ; Disable Output Endpoint-4 interrupt
  eint                                ; Enable all interrupts
  mov #OS_IO_USB_OUTPUT_ENDPOINT4,r13 ; Load interrupt entry to Escapement device handler table in R13
  jmp EndUSB
USB_OUTPUT_ENDPOINT3:
  bic.b #BIT3,&USBOEPIE               ; Disable Output Endpoint-3 interrupt
  eint                                ; Enable all interrupts
  mov #OS_IO_USB_OUTPUT_ENDPOINT3,r13 ; Load interrupt entry to Escapement device handler table in R13
  jmp EndUSB
USB_OUTPUT_ENDPOINT2:
  bic.b #BIT2,&USBOEPIE               ; Disable Output Endpoint-2 interrupt
  eint                                ; Enable all interrupts
  mov #OS_IO_USB_OUTPUT_ENDPOINT2,r13 ; Load interrupt entry to Escapement device handler table in R13
  jmp EndUSB
USB_OUTPUT_ENDPOINT1:
  bic.b #BIT1,&USBOEPIE               ; Disable Output Endpoint-1 interrupt
  eint                                ; Enable all interrupts
  mov #OS_IO_USB_OUTPUT_ENDPOINT1,r13 ; Load interrupt entry to Escapement device handler table in R13
  jmp EndUSB
USB_INPUT_ENDPOINT7:
  bic.b #BIT7,&USBIEPIE               ; Disable Input Endpoint-7 interrupt
  eint                                ; Enable all interrupts
  mov #OS_IO_USB_INPUT_ENDPOINT7,r13  ; Load interrupt entry to Escapement device handler table in R13
  jmp EndUSB
USB_INPUT_ENDPOINT6:
  bic.b #BIT6,&USBIEPIE               ; Disable Input Endpoint-6 interrupt
  eint                                ; Enable all interrupts
  mov #OS_IO_USB_INPUT_ENDPOINT6,r13  ; Load interrupt entry to Escapement device handler table in R13
  jmp EndUSB
USB_INPUT_ENDPOINT5:
  bic.b #BIT5,&USBIEPIE               ; Disable Input Endpoint-5 interrupt
  eint                                ; Enable all interrupts
  mov #OS_IO_USB_INPUT_ENDPOINT5,r13  ; Load interrupt entry to Escapement device handler table in R13
  jmp EndUSB
USB_INPUT_ENDPOINT4:
  bic.b #BIT4,&USBIEPIE               ; Disable Input Endpoint-4 interrupt
  eint                                ; Enable all interrupts
  mov #OS_IO_USB_INPUT_ENDPOINT4,r13  ; Load interrupt entry to Escapement device handler table in R13
  jmp EndUSB
USB_INPUT_ENDPOINT3:
  bic.b #BIT3,&USBIEPIE               ; Disable Input Endpoint-3 interrupt
  eint                                ; Enable all interrupts
  mov #OS_IO_USB_INPUT_ENDPOINT3,r13  ; Load interrupt entry to Escapement device handler table in R13
  jmp EndUSB
USB_INPUT_ENDPOINT2:
  bic.b #BIT2,&USBIEPIE               ; Disable Input Endpoint-2 interrupt
  eint                                ; Enable all interrupts
  mov #OS_IO_USB_INPUT_ENDPOINT2,r13  ; Load interrupt entry to Escapement device handler table in R13
  jmp EndUSB
USB_INPUT_ENDPOINT1:
  bic.b #BIT1,&USBIEPIE               ; Disable Input Endpoint-1 interrupt
  eint                                ; Enable all interrupts
  mov #OS_IO_USB_INPUT_ENDPOINT1,r13  ; Load interrupt entry to Escapement device handler table in R13
  jmp EndUSB
USB_STPOW_PACKET_RECEIVED: 
  bic.b #STPOWIE,&USBIE               ; Disable interrupt
  eint                                ; Enable all interrupts
  mov #OS_IO_USB_STPOW_PACKET_RECEIVED,r13  ; Load interrupt entry to Escapement device handler table in R13
  jmp EndUSB
...
USB_RESR:
  ...
  eint                                ; Enable all interrupts
  mov #OS_IO_USB_RESR,r13             ; Load interrupt entry to Escapement device handler table in R13
  jmp EndUSB
USB_SUSR:
  bic.b #SUSRIE,&USBIE                ; Disable suspend interrupt
  eint                                ; Enable all interrupts
  mov #OS_IO_USB_SUSR,r13             ; Load interrupt entry to Escapement device handler table in R13
  jmp EndUSB
USB_RSTR:
  bic.b #RSTRIE,&USBIE                ; Disable reset interrupt
  eint                                ; Enable all interrupts
  mov #OS_IO_USB_RSTR,r13             ; Load interrupt entry to Escapement device handler table in R13
  jmp EndUSB
USB_OUTPUT_ENDPOINT0:
  bic.b #BIT0,&USBOEPIE               ; Disable Output Endpoint-0 interrupt
  eint                                ; Enable all interrupts
  mov #OS_IO_USB_OUTPUT_ENDPOINT0,r13 ; Load interrupt entry to Escapement device handler table in R13
  jmp EndUSB
USB_INPUT_ENDPOINT0:
  bic.b #BIT0,&USBIEPIE               ; Disable Input Endpoint-0 interrupt
  eint                                ; Enable all interrupts
  mov #OS_IO_USB_INPUT_ENDPOINT0,r13  ; Load interrupt entry to Escapement device handler table in R13
  jmp EndUSB
USB_USB_TIMESTAMP:
  bic #UTIE,&USBMAINT                 ; Disable USB timestamp event interrupt
  eint                                ; Enable all interrupts
  mov #OS_IO_USB_USB_TIMESTAMP,r13    ; Load interrupt entry to Escapement device handler table in R13
  jmp EndUSB
USB_PWR_VBUSOff:
  ...
  eint                                ; Enable all interrupts
  mov #OS_IO_USB_PWR_VBUSOff,r13      ; Load interrupt entry to Escapement device handler table in R13
  jmp EndUSB
USB_PWR_VBUSOn:
  mov #0x9628,&USBKEYPID              ; set KEY and PID to 0x9628 -> access to configuration registers enabled
  ...
  mov #0x9600,&USBKEYPID              ; access to configuration registers disabled
  eint                                ; Enable all interrupts
  mov #OS_IO_USB_PWR_VBUSOn,r13       ; Load interrupt entry to Escapement device handler table in R13
  jmp EndUSB
USB_PLL_RANGE:
  mov #0x9628,&USBKEYPID              ; set KEY and PID to 0x9628 -> access to configuration registers enabled
  ...
  mov #0x9600,&USBKEYPID              ; access to configuration registers disabled
  eint                                ; Enable all interrupts
  mov #OS_IO_USB_PLL_RANGE,r13        ; Load interrupt entry to Escapement device handler table in R13
  jmp EndUSB
USB_PLL_SIGNAL:
  mov #0x9628,&USBKEYPID              ; set KEY and PID to 0x9628 -> access to configuration registers enabled
  ...
  mov #0x9600,&USBKEYPID              ; access to configuration registers disabled
  eint                                ; Enable all interrupts
  mov #OS_IO_USB_PLL_SIGNAL,r13       ; Load interrupt entry to Escapement device handler table in R13
  jmp EndUSB
USB_PLL_LOCK:
  mov #0x9628,&USBKEYPID              ; set KEY and PID to 0x9628 -> access to configuration registers enabled
  ...
  mov #0x9600,&USBKEYPID              ; access to configuration registers disabled
  eint                                ; Enable all interrupts
  mov #OS_IO_USB_PLL_LOCK,r13         ; Load interrupt entry to Escapement device handler table in R13
  jmp EndUSB
USB_PWR_DROP:
  mov #0x9628,&USBKEYPID              ; set KEY and PID to 0x9628 -> access to configuration registers enabled
  ...
  mov #0x9600,&USBKEYPID              ; access to configuration registers disabled
  eint                                ; Enable all interrupts
  mov #OS_IO_USB_PWR_DROP,r13         ; Load interrupt entry to Escapement device handler table in R13
  jmp EndUSB
USB_NO_INTERRUPT:
  eint                                ; Enable all interrupts
  mov #OS_IO_USB_NO_INTERRUPT,r13     ; Load interrupt entry to Escapement device handler table in R13
EndUSB:
  SaveCtx                             ; Save context of the application task
  mov #_OSTabDevice,r12               ; Load Escapement device handler table in R12
...
; the ISR and restored when exiting the ISR.
  ...
  .endif
; Vector 16 (Port 2 pin 7): Escapement system clock timer.
...
OSAlloca_InfinitLoop:
     jmp OSAlloca_InfinitLoop       ; Memory overflow: increase the value of OSALLOCA_IN-
                                    ; TERNAL_HEAP_SIZE defined in Escapement_msp430xxx.h
  .else
