...
*/
/* File Escapement_2xx.h: Defines the user interface with Escapement.c for TI MSP430F2X family.
** Version identifier: July 2008
*/
...
**       int main(void)
**       {
**          // do some initializations here and then create a task, passing it the value
**          // 34 as a parameter.
**          OSCreateTask(Task1,100,0,10000,10000,1,1,0,(void *)34);
...
**       } // end of Task1
**
** Besides periodic tasks, Escapement also supports tasks with irregular arrival times that
...
** they are called sporadic tasks. Only sporadic tasks are supported by Escapement and they
...
** ted, an aperiodic task becomes blocked until it is signalled. Signalling is done via
** function OSScheduleSuspendedTask(eventId). Once signalled a new instance is created
** and scheduled.
...
**          OSCreateTask(SignalerTask,100,0,10000,10000,1,1,0,NULL);
...
** OSAlloca, and the second is when an event-driven task overruns its period (under Dead-
** line Monotonic Scheduling) or its allotted load (under EDF) and is immediately resche-
** duled. This last case is caused when the instantaneous load of a processor is higher
** than 100%. The third and final error source is caused when configurating the number of
** priority levels of the interrupt system.
...
/* Escapement can work under 2 possible scheduling algorithms. Under EDF, tasks are sche-
...
** static priority based on their deadline. This priorities are determined on-line prior
...
/* MISCELLANEOUS FUNCTIONS: LAUNCHING OF ESCAPEMENT, MEMORY MANAGEMENT */
...
/* OSAlloca: This function replaces a regular malloc when used before calling OSStart-
** Multitasking. Memory allocations performed by this function can never be freed
** once allocated. The purpose of this function is to set-up memory blocks prior to
** executing the application tasks without having the burden of having a malloc
** function that keeps track of the free blocks.
** Important: On the MSP430 line of microcontrollers, this function should NEVER be
** used in the application tasks or in user defined ISRs.
** Parameter: (UINT16) requested memory block size in bytes.
** Returned value: (void *) Pointer to the base of the newly allocated block, and NULL
** in case of an error, i.e., when more than half of the memory is consumed by dy-
** namic allocations done in main. */
void *OSAlloca(UINT16 size);
...
/* OSEndTask: Very last instruction in an application task. The purpose of this call is
** to schedule the next instance of the task. Any code after the call will never be exe-
** cuted. Also, without this call, the task instance can never terminate. */
void OSEndTask(void);
...
** tion to block and wake-up a task instance.
** Returned value: NULL when a memory failure occurs, or a valid descriptor. */
void *OSCreateEventDescriptor(void);

/* OSCreateSynchronousTask: Like OSCreateTask() but applied to event-driven (synchronous)
** tasks.
** Parameters:
**   (1) Pointer to the task's code;
**   (2) (INT32) workload: equal to WCET / (available processor load); this is the same
**       as the period and deadline of the task; under EDF this task is scheduled when
**       all previous synchronous tasks have completed, but under deadline monotonic
**       scheduling, this task behaves like any other task;
**   (3) (void *) event: Descriptor returned by OSCreateSynchronousTask();
**   (4) (void *) argument: An instance specific 32-bit value.
** Returned value: TRUE if the task creation was successful and FALSE otherwise. The
**   function fails when there's a memory allocation failure. */
...
** tion checks if the calling instance should restart a new instance (an event was sig-
** naled) or if it should suspend itself until the event is signaled by another task
...
** dules it. This function acts as a memorized signal. If there are no suspended tasks at
** the moment of the call, the signal is stored and when an aperiodic task terminates, it
** is immediately rescheduled. An aperiodic task can therefore signal itself.
** Parameter: (void *) event descriptor holding the blocked tasks. */
void OSScheduleSuspendedTask(void *eventQueue);


/* I/O DEVICE VECTORING -------------------------------------------------------------- */
/* Associated with each I/O device is a memory location called the interrupt vector that
** contains the address of a descriptor for each I/O device. The minimal content of the
** descriptor is the address of the interrupt service procedure:
...
** is then given to the interrupt service routine specified within the descriptor. This
** is equivalent to the following code snippet:
**     interruptVector[sourceEntry]->myInterruptHandler(&interruptVector[sourceEntry])
** Because the interrupt service procedure receives a pointer to the I/O descriptor, it
** can store in that descriptor all the information that is needed to process the I/O
** without resorting to global or static variables. The last instruction to performed by
** interrupt service procedure must be OSEndInterrupt(), which restores the saved regis-
** ters of the interrupted task instance before resuming the instance:
**     void myInterruptHandler(struct myDescriptorDef *myDescriptor)
**     {
**        // process the interrupt
**        OSEndInterrupt(); // return to the interrupted task instance
**     }
** On the MSP430F5xx, some I/O devices also have their own interrupt vector that can be
** accessible via a dedicated register. We call these devices multiple-sourced devices.
** The only difference with a single-sourced device is the prototype of the interrupt
** service routine, which receives an extra parameter, the contents of the peripheral's
** vector register. In this case, the I/O descriptor should be defined as
**     typedef struct myDescriptorDef {
**        void (*myInterruptHandler)(struct myDescriptorDef *, UINT16);
**        // other information used by myInterruptHandler
**     } myDescriptorDef;
** 
** We do not provide a descriptor template because it is device dependent.
**
...
/* OSSetIODescriptor: Associates an interrupt service descriptor with an I/O device. The
** very first field of the descriptor is a pointer to an interrupt service procedure that
** is called when an interrupt from the device should be handled.
** Parameters:
**   (1) (UINT8) Entry to I/O interrupt vector;
**   (2) (void *) I/O descriptor for the device. */
void OSSetIODescriptor(UINT8 portIndex, void *descriptor);
...
** tor can be returned to the user in order to read or to store his application data.
** Parameter: (UINT8) Index in the I/O device interrupt vector where the I/O descriptor
**    is held.
** Returned value: (void *) The requested I/O descriptor is returned. If no previous call
**    to OSSetIODescriptor was previously made for the specified I/O port, the returned
**    value is undefined. */
void *OSGetIODescriptor(UINT8 portIndex);

/* OSEndInterrupt: Very last instruction in an interrupt service procedure. The purpose
** of the call is to resume in the interrupted task instance. Any code after the call
** will never be executed. Also, without this call, the interrupt service procedure can
** never terminate. */
void OSEndInterrupt(void);


/* INTER-TASK COMMUNICATION AND I/O BUFFERS FOR INTERRUPT HANDLER -------------------- */
...
** digm is provided in Escapement and described below.
...
** like Escapement, we also chose to provide an API to allow concurrent, and thus asynchro-
...
** responding SC functions call to provide synchronization support for Escapement. The LL/S
...
** The LL function is semantically equivalent to the atomic execution of the following code:
...
**    rameter.
** Note that this function is emulated in Escapement_a_5xx.asm. */
